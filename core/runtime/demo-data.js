...
// Demo/UX fixture data shared by the Home "Load demo data" action and
// scripts/seed-ux-test-data.js. Every row id is prefixed (ux_mem_, ux_line_,
// ux_trace_, ...) so clearDemoFixture can remove exactly what was seeded.
...
async function clearDemoFixture(database) {
  ...
  await database.exec(`
    ...
    DELETE FROM innerlife_profiles WHERE agent_id IN ('clara', 'lara') AND state_json LIKE '%"uxFixture":true%';
...
async function seedDemoFixture(database, { dataRoot = "" } = {}) {
  const now = ts(0);
  await database.exec(`
    ...
    VALUES
      ('clara', 'Clara', 1, ${jsonSql({ watch: ["handoff clarity", "operator fatigue"], avoid: ["over-explaining normal state"] })}, ${jsonSql({ uxFixture: true, recent_focus: "Home reduction review", mood: "focused" })}, ${sqlString(ts(200))}, ${sqlString(ts(14))}),
      ('lara', 'Lara', 1, ${jsonSql({ watch: ["Gateway errors", "setup copy clarity"], avoid: ["hidden ports"] })}, ${jsonSql({ uxFixture: true, recent_focus: "Agent Access next pass", mood: "alert" })}, ${sqlString(ts(190))}, ${sqlString(ts(22))})
    ...
    VALUES
      ('ux_event_seed_complete', 'info', 'desktop', 'UX test data seeded', ${sqlString(now)}, ${jsonSql({ fixture: true, dataRoot })}),
...
module.exports = { seedDemoFixture, clearDemoFixture };
