...
- `recentMemories`: the single recent active Memory sample, limit 20.
  `memories` is view-owned paging/filter state and must not be duplicated in
  the overview.
...
- `innerLife`: lite counts, daemon/doctor state, profiles, and bounded waiting
  share previews only. Waiting inbox rows use the single `inbox` key;
  `pendingInbox` remains an InnerLife-domain lite-snapshot field, not a second
  overview alias.
...
The maintained adversarial Overview ceiling is 135 KiB. Runtime-change scopes
invalidate only their owning views and derived consumers. A compact Overview
refresh preserves already-hydrated detail owned by unaffected views; snapshot-
only or unknown scopes conservatively invalidate every view. Memory and Agent
Setup have no generic view-detail IPC and must not issue an empty
`getViewSnapshot()` call.
...
The configured maintenance hour accepts the full local-hour range, including
`00:00`. If Desktop starts after a missed run, it performs one immediate
catch-up. The success watermark advances only after every maintenance and
retention step completes. A failed catch-up or scheduled run retries with
exponential backoff from five minutes, capped at six hours, so a persistent
failure cannot create a zero-delay loop.
